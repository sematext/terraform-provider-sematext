...
// ResourceTestFixtureElasticsearch TODO Doc Comment
struct ResourceTestFixtureElasticsearch {
	Name string
	Description string
	Plan string
...
func (rtf *ResourceTestFixtureElasticsearch) HydrateBasic() (*ResourceTestFixtureElasticsearch){
	rndID := acctest.RandStringFromCharSet(10, acctest.CharSetAlphaNum)
	rtf.Name = strings.ToLowercase(fmt.Sprintf("Elasticsearch_test_%s", rndID))
	rtf.Description = "TESTING : SematextMonitorElasticsearch_Basic : Create"
	rtf.Plan = "basic"
...
func (rtf *ResourceTestFixtureElasticsearch) FormatToHCL() {

	monitortype = strings.ToLowerCase("Elasticsearch")
...
// testAccSematextMonitorElasticsearch_Basic tests resource creation.
func testAccSematextMonitorElasticsearchBasic(t *testing.T) {

	monitortype = strings.ToLowerCase("Elasticsearch")
	rtf := (&ResourceTestFixtureElasticsearch{}).HydrateBasic()
	...
	resource.Test(t, resource.TestCase{
		...
		CheckDestroy: testAccSematextMonitorElasticsearch_ConfirmDestroyed,
		Steps: []resource.TestStep{
			{
				Config: fixture,
				Check: resource.ComposeTestCheckFunc(
					testAccSematextMonitorElasticsearch_CheckConsistency(name),
...
// testAccSematextMonitorElasticsearch_Update tests for resource updates.
func testAccSematextMonitorElasticsearchUpdate(t *testing.T) {

	monitortype = strings.ToLowerCase("Elasticsearch")

	rtf1 := (&ResourceTestFixtureElasticsearch{}).HydrateBasic()
	statepath := fmt.Sprintf("sematext_monitor_%s.%s", monitortype, rtf1.Name)

	rtf2 := rtf1
	rtf2.Description = "TESTING : SematextMonitorElasticsearch_Basic : Update"
		...
		Steps: []resource.TestStep{
			{
				Config: fixture1,
				Check: resource.ComposeTestCheckFunc(
					testAccSematextMonitorElasticsearchCheckConsistency(rtf1.Name),
			...
			},
			{
				Config: fixture2,
				Check: resource.ComposeTestCheckFunc(
					testAccSematextMonitorElasticsearch_CheckConsistency(rtf2.name),
...
// testAccSematextMonitorElasticsearch_CheckConsistency checks the App ID exists in both state and API.
func testAccSematextMonitorElasticsearchCheckConsistency(name string) resource.TestCheckFunc {
...
// testAccSematextMonitorElasticsearch_ConfirmDestroyed -  check is destroyed in API
func testAccSematextMonitorElasticsearchConfirmDestroyed(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		...
		if rs.Type != "sematext_monitor_Elasticsearch" {
			continue
		}
