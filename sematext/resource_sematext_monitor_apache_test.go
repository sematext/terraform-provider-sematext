...
// ResourceTestFixtureApache TODO Doc Comment
struct ResourceTestFixtureApache {
	Name string
	Description string
	Plan string
...
func (rtf *ResourceTestFixtureApache) HydrateBasic() (*ResourceTestFixtureApache){
	rndID := acctest.RandStringFromCharSet(10, acctest.CharSetAlphaNum)
	rtf.Name = strings.ToLowercase(fmt.Sprintf("Apache_test_%s", rndID))
	rtf.Description = "TESTING : SematextMonitorApache_Basic : Create"
	rtf.Plan = "basic"
...
func (rtf *ResourceTestFixtureApache) FormatToHCL() {

	monitortype = strings.ToLowerCase("Apache")
...
// testAccSematextMonitorApache_Basic tests resource creation.
func testAccSematextMonitorApacheBasic(t *testing.T) {

	monitortype = strings.ToLowerCase("Apache")
	rtf := (&ResourceTestFixtureApache{}).HydrateBasic()
	...
	resource.Test(t, resource.TestCase{
		...
		CheckDestroy: testAccSematextMonitorApache_ConfirmDestroyed,
		Steps: []resource.TestStep{
			{
				Config: fixture,
				Check: resource.ComposeTestCheckFunc(
					testAccSematextMonitorApache_CheckConsistency(name),
...
// testAccSematextMonitorApache_Update tests for resource updates.
func testAccSematextMonitorApacheUpdate(t *testing.T) {

	monitortype = strings.ToLowerCase("Apache")

	rtf1 := (&ResourceTestFixtureApache{}).HydrateBasic()
	statepath := fmt.Sprintf("sematext_monitor_%s.%s", monitortype, rtf1.Name)

	rtf2 := rtf1
	rtf2.Description = "TESTING : SematextMonitorApache_Basic : Update"
		...
		Steps: []resource.TestStep{
			{
				Config: fixture1,
				Check: resource.ComposeTestCheckFunc(
					testAccSematextMonitorApacheCheckConsistency(rtf1.Name),
			...
			},
			{
				Config: fixture2,
				Check: resource.ComposeTestCheckFunc(
					testAccSematextMonitorApache_CheckConsistency(rtf2.name),
...
// testAccSematextMonitorApache_CheckConsistency checks the App ID exists in both state and API.
func testAccSematextMonitorApacheCheckConsistency(name string) resource.TestCheckFunc {
...
// testAccSematextMonitorApache_ConfirmDestroyed -  check is destroyed in API
func testAccSematextMonitorApacheConfirmDestroyed(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		...
		if rs.Type != "sematext_monitor_Apache" {
			continue
		}
