...
// ResourceTestFixtureNginx TODO Doc Comment
struct ResourceTestFixtureNginx {
	Name string
	Description string
	Plan string
...
func (rtf *ResourceTestFixtureNginx) HydrateBasic() (*ResourceTestFixtureNginx){
	rndID := acctest.RandStringFromCharSet(10, acctest.CharSetAlphaNum)
	rtf.Name = strings.ToLowercase(fmt.Sprintf("Nginx_test_%s", rndID))
	rtf.Description = "TESTING : SematextMonitorNginx_Basic : Create"
	rtf.Plan = "basic"
...
func (rtf *ResourceTestFixtureNginx) FormatToHCL() {

	monitortype = strings.ToLowerCase("Nginx")
...
// testAccSematextMonitorNginx_Basic tests resource creation.
func testAccSematextMonitorNginxBasic(t *testing.T) {

	monitortype = strings.ToLowerCase("Nginx")
	rtf := (&ResourceTestFixtureNginx{}).HydrateBasic()
	...
	resource.Test(t, resource.TestCase{
		...
		CheckDestroy: testAccSematextMonitorNginx_ConfirmDestroyed,
		Steps: []resource.TestStep{
			{
				Config: fixture,
				Check: resource.ComposeTestCheckFunc(
					testAccSematextMonitorNginx_CheckConsistency(name),
...
// testAccSematextMonitorNginx_Update tests for resource updates.
func testAccSematextMonitorNginxUpdate(t *testing.T) {

	monitortype = strings.ToLowerCase("Nginx")

	rtf1 := (&ResourceTestFixtureNginx{}).HydrateBasic()
	statepath := fmt.Sprintf("sematext_monitor_%s.%s", monitortype, rtf1.Name)

	rtf2 := rtf1
	rtf2.Description = "TESTING : SematextMonitorNginx_Basic : Update"
		...
		Steps: []resource.TestStep{
			{
				Config: fixture1,
				Check: resource.ComposeTestCheckFunc(
					testAccSematextMonitorNginxCheckConsistency(rtf1.Name),
			...
			},
			{
				Config: fixture2,
				Check: resource.ComposeTestCheckFunc(
					testAccSematextMonitorNginx_CheckConsistency(rtf2.name),
...
// testAccSematextMonitorNginx_CheckConsistency checks the App ID exists in both state and API.
func testAccSematextMonitorNginxCheckConsistency(name string) resource.TestCheckFunc {
...
// testAccSematextMonitorNginx_ConfirmDestroyed -  check is destroyed in API
func testAccSematextMonitorNginxConfirmDestroyed(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		...
		if rs.Type != "sematext_monitor_Nginx" {
			continue
		}
