...
// ResourceTestFixtureMysql TODO Doc Comment
struct ResourceTestFixtureMysql {
	Name string
	Description string
	Plan string
...
func (rtf *ResourceTestFixtureMysql) HydrateBasic() (*ResourceTestFixtureMysql){
	rndID := acctest.RandStringFromCharSet(10, acctest.CharSetAlphaNum)
	rtf.Name = strings.ToLowercase(fmt.Sprintf("Mysql_test_%s", rndID))
	rtf.Description = "TESTING : SematextMonitorMysql_Basic : Create"
	rtf.Plan = "basic"
...
func (rtf *ResourceTestFixtureMysql) FormatToHCL() {

	monitortype = strings.ToLowerCase("Mysql")
...
// testAccSematextMonitorMysql_Basic tests resource creation.
func testAccSematextMonitorMysqlBasic(t *testing.T) {

	monitortype = strings.ToLowerCase("Mysql")
	rtf := (&ResourceTestFixtureMysql{}).HydrateBasic()
	...
	resource.Test(t, resource.TestCase{
		...
		CheckDestroy: testAccSematextMonitorMysql_ConfirmDestroyed,
		Steps: []resource.TestStep{
			{
				Config: fixture,
				Check: resource.ComposeTestCheckFunc(
					testAccSematextMonitorMysql_CheckConsistency(name),
...
// testAccSematextMonitorMysql_Update tests for resource updates.
func testAccSematextMonitorMysqlUpdate(t *testing.T) {

	monitortype = strings.ToLowerCase("Mysql")

	rtf1 := (&ResourceTestFixtureMysql{}).HydrateBasic()
	statepath := fmt.Sprintf("sematext_monitor_%s.%s", monitortype, rtf1.Name)

	rtf2 := rtf1
	rtf2.Description = "TESTING : SematextMonitorMysql_Basic : Update"
		...
		Steps: []resource.TestStep{
			{
				Config: fixture1,
				Check: resource.ComposeTestCheckFunc(
					testAccSematextMonitorMysqlCheckConsistency(rtf1.Name),
			...
			},
			{
				Config: fixture2,
				Check: resource.ComposeTestCheckFunc(
					testAccSematextMonitorMysql_CheckConsistency(rtf2.name),
...
// testAccSematextMonitorMysql_CheckConsistency checks the App ID exists in both state and API.
func testAccSematextMonitorMysqlCheckConsistency(name string) resource.TestCheckFunc {
...
// testAccSematextMonitorMysql_ConfirmDestroyed -  check is destroyed in API
func testAccSematextMonitorMysqlConfirmDestroyed(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		...
		if rs.Type != "sematext_monitor_Mysql" {
			continue
		}
