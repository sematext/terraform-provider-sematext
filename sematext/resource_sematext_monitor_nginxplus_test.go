...
// ResourceTestFixtureNginxplus TODO Doc Comment
struct ResourceTestFixtureNginxplus {
	Name string
	Description string
	Plan string
...
func (rtf *ResourceTestFixtureNginxplus) HydrateBasic() (*ResourceTestFixtureNginxplus){
	rndID := acctest.RandStringFromCharSet(10, acctest.CharSetAlphaNum)
	rtf.Name = strings.ToLowercase(fmt.Sprintf("Nginxplus_test_%s", rndID))
	rtf.Description = "TESTING : SematextMonitorNginxplus_Basic : Create"
	rtf.Plan = "basic"
...
func (rtf *ResourceTestFixtureNginxplus) FormatToHCL() {

	monitortype = strings.ToLowerCase("Nginxplus")
...
// testAccSematextMonitorNginxplus_Basic tests resource creation.
func testAccSematextMonitorNginxplusBasic(t *testing.T) {

	monitortype = strings.ToLowerCase("Nginxplus")
	rtf := (&ResourceTestFixtureNginxplus{}).HydrateBasic()
	...
	resource.Test(t, resource.TestCase{
		...
		CheckDestroy: testAccSematextMonitorNginxplus_ConfirmDestroyed,
		Steps: []resource.TestStep{
			{
				Config: fixture,
				Check: resource.ComposeTestCheckFunc(
					testAccSematextMonitorNginxplus_CheckConsistency(name),
...
// testAccSematextMonitorNginxplus_Update tests for resource updates.
func testAccSematextMonitorNginxplusUpdate(t *testing.T) {

	monitortype = strings.ToLowerCase("Nginxplus")

	rtf1 := (&ResourceTestFixtureNginxplus{}).HydrateBasic()
	statepath := fmt.Sprintf("sematext_monitor_%s.%s", monitortype, rtf1.Name)

	rtf2 := rtf1
	rtf2.Description = "TESTING : SematextMonitorNginxplus_Basic : Update"
		...
		Steps: []resource.TestStep{
			{
				Config: fixture1,
				Check: resource.ComposeTestCheckFunc(
					testAccSematextMonitorNginxplusCheckConsistency(rtf1.Name),
			...
			},
			{
				Config: fixture2,
				Check: resource.ComposeTestCheckFunc(
					testAccSematextMonitorNginxplus_CheckConsistency(rtf2.name),
...
// testAccSematextMonitorNginxplus_CheckConsistency checks the App ID exists in both state and API.
func testAccSematextMonitorNginxplusCheckConsistency(name string) resource.TestCheckFunc {
...
// testAccSematextMonitorNginxplus_ConfirmDestroyed -  check is destroyed in API
func testAccSematextMonitorNginxplusConfirmDestroyed(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		...
		if rs.Type != "sematext_monitor_Nginxplus" {
			continue
		}
