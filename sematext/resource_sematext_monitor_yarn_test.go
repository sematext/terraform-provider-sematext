...
// ResourceTestFixtureYarn TODO Doc Comment
struct ResourceTestFixtureYarn {
	Name string
	Description string
	Plan string
...
func (rtf *ResourceTestFixtureYarn) HydrateBasic() (*ResourceTestFixtureYarn){
	rndID := acctest.RandStringFromCharSet(10, acctest.CharSetAlphaNum)
	rtf.Name = strings.ToLowercase(fmt.Sprintf("Yarn_test_%s", rndID))
	rtf.Description = "TESTING : SematextMonitorYarn_Basic : Create"
	rtf.Plan = "basic"
...
func (rtf *ResourceTestFixtureYarn) FormatToHCL() {

	monitortype = strings.ToLowerCase("Yarn")
...
// testAccSematextMonitorYarn_Basic tests resource creation.
func testAccSematextMonitorYarnBasic(t *testing.T) {

	monitortype = strings.ToLowerCase("Yarn")
	rtf := (&ResourceTestFixtureYarn{}).HydrateBasic()
	...
	resource.Test(t, resource.TestCase{
		...
		CheckDestroy: testAccSematextMonitorYarn_ConfirmDestroyed,
		Steps: []resource.TestStep{
			{
				Config: fixture,
				Check: resource.ComposeTestCheckFunc(
					testAccSematextMonitorYarn_CheckConsistency(name),
...
// testAccSematextMonitorYarn_Update tests for resource updates.
func testAccSematextMonitorYarnUpdate(t *testing.T) {

	monitortype = strings.ToLowerCase("Yarn")

	rtf1 := (&ResourceTestFixtureYarn{}).HydrateBasic()
	statepath := fmt.Sprintf("sematext_monitor_%s.%s", monitortype, rtf1.Name)

	rtf2 := rtf1
	rtf2.Description = "TESTING : SematextMonitorYarn_Basic : Update"
		...
		Steps: []resource.TestStep{
			{
				Config: fixture1,
				Check: resource.ComposeTestCheckFunc(
					testAccSematextMonitorYarnCheckConsistency(rtf1.Name),
			...
			},
			{
				Config: fixture2,
				Check: resource.ComposeTestCheckFunc(
					testAccSematextMonitorYarn_CheckConsistency(rtf2.name),
...
// testAccSematextMonitorYarn_CheckConsistency checks the App ID exists in both state and API.
func testAccSematextMonitorYarnCheckConsistency(name string) resource.TestCheckFunc {
...
// testAccSematextMonitorYarn_ConfirmDestroyed -  check is destroyed in API
func testAccSematextMonitorYarnConfirmDestroyed(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		...
		if rs.Type != "sematext_monitor_Yarn" {
			continue
		}
