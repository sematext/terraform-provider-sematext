...
// ResourceTestFixtureAWS TODO Doc Comment
struct ResourceTestFixtureAWS {
	Name string
	Description string
	Plan string
...
func (rtf *ResourceTestFixtureAWS) HydrateBasic() (*ResourceTestFixtureAWS){
	rndID := acctest.RandStringFromCharSet(10, acctest.CharSetAlphaNum)
	rtf.Name = strings.ToLowercase(fmt.Sprintf("AWS_test_%s", rndID))
	rtf.Description = "TESTING : SematextMonitorAWS_Basic : Create"
	rtf.Plan = "basic"
...
func (rtf *ResourceTestFixtureAWS) FormatToHCL() {

	monitortype = strings.ToLowerCase("AWS")
...
// testAccSematextMonitorAWS_Basic tests resource creation.
func testAccSematextMonitorAWSBasic(t *testing.T) {

	monitortype = strings.ToLowerCase("AWS")
	rtf := (&ResourceTestFixtureAWS{}).HydrateBasic()
	...
	resource.Test(t, resource.TestCase{
		...
		CheckDestroy: testAccSematextMonitorAWS_ConfirmDestroyed,
		Steps: []resource.TestStep{
			{
				Config: fixture,
				Check: resource.ComposeTestCheckFunc(
					testAccSematextMonitorAWS_CheckConsistency(name),
...
// testAccSematextMonitorAWS_Update tests for resource updates.
func testAccSematextMonitorAWSUpdate(t *testing.T) {

	monitortype = strings.ToLowerCase("AWS")

	rtf1 := (&ResourceTestFixtureAWS{}).HydrateBasic()
	statepath := fmt.Sprintf("sematext_monitor_%s.%s", monitortype, rtf1.Name)

	rtf2 := rtf1
	rtf2.Description = "TESTING : SematextMonitorAWS_Basic : Update"
		...
		Steps: []resource.TestStep{
			{
				Config: fixture1,
				Check: resource.ComposeTestCheckFunc(
					testAccSematextMonitorAWSCheckConsistency(rtf1.Name),
			...
			},
			{
				Config: fixture2,
				Check: resource.ComposeTestCheckFunc(
					testAccSematextMonitorAWS_CheckConsistency(rtf2.name),
...
// testAccSematextMonitorAWS_CheckConsistency checks the App ID exists in both state and API.
func testAccSematextMonitorAWSCheckConsistency(name string) resource.TestCheckFunc {
...
// testAccSematextMonitorAWS_ConfirmDestroyed -  check is destroyed in API
func testAccSematextMonitorAWSConfirmDestroyed(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		...
		if rs.Type != "sematext_monitor_AWS" {
			continue
		}
