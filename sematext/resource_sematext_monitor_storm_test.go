...
// ResourceTestFixtureStorm TODO Doc Comment
struct ResourceTestFixtureStorm {
	Name string
	Description string
	Plan string
...
func (rtf *ResourceTestFixtureStorm) HydrateBasic() (*ResourceTestFixtureStorm){
	rndID := acctest.RandStringFromCharSet(10, acctest.CharSetAlphaNum)
	rtf.Name = strings.ToLowercase(fmt.Sprintf("Storm_test_%s", rndID))
	rtf.Description = "TESTING : SematextMonitorStorm_Basic : Create"
	rtf.Plan = "basic"
...
func (rtf *ResourceTestFixtureStorm) FormatToHCL() {

	monitortype = strings.ToLowerCase("Storm")
...
// testAccSematextMonitorStorm_Basic tests resource creation.
func testAccSematextMonitorStormBasic(t *testing.T) {

	monitortype = strings.ToLowerCase("Storm")
	rtf := (&ResourceTestFixtureStorm{}).HydrateBasic()
	...
	resource.Test(t, resource.TestCase{
		...
		CheckDestroy: testAccSematextMonitorStorm_ConfirmDestroyed,
		Steps: []resource.TestStep{
			{
				Config: fixture,
				Check: resource.ComposeTestCheckFunc(
					testAccSematextMonitorStorm_CheckConsistency(name),
...
// testAccSematextMonitorStorm_Update tests for resource updates.
func testAccSematextMonitorStormUpdate(t *testing.T) {

	monitortype = strings.ToLowerCase("Storm")

	rtf1 := (&ResourceTestFixtureStorm{}).HydrateBasic()
	statepath := fmt.Sprintf("sematext_monitor_%s.%s", monitortype, rtf1.Name)

	rtf2 := rtf1
	rtf2.Description = "TESTING : SematextMonitorStorm_Basic : Update"
		...
		Steps: []resource.TestStep{
			{
				Config: fixture1,
				Check: resource.ComposeTestCheckFunc(
					testAccSematextMonitorStormCheckConsistency(rtf1.Name),
			...
			},
			{
				Config: fixture2,
				Check: resource.ComposeTestCheckFunc(
					testAccSematextMonitorStorm_CheckConsistency(rtf2.name),
...
// testAccSematextMonitorStorm_CheckConsistency checks the App ID exists in both state and API.
func testAccSematextMonitorStormCheckConsistency(name string) resource.TestCheckFunc {
...
// testAccSematextMonitorStorm_ConfirmDestroyed -  check is destroyed in API
func testAccSematextMonitorStormConfirmDestroyed(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		...
		if rs.Type != "sematext_monitor_Storm" {
			continue
		}
