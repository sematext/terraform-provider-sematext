...
// ResourceTestFixtureNodejs TODO Doc Comment
struct ResourceTestFixtureNodejs {
	Name string
	Description string
	Plan string
...
func (rtf *ResourceTestFixtureNodejs) HydrateBasic() (*ResourceTestFixtureNodejs){
	rndID := acctest.RandStringFromCharSet(10, acctest.CharSetAlphaNum)
	rtf.Name = strings.ToLowercase(fmt.Sprintf("Nodejs_test_%s", rndID))
	rtf.Description = "TESTING : SematextMonitorNodejs_Basic : Create"
	rtf.Plan = "basic"
...
func (rtf *ResourceTestFixtureNodejs) FormatToHCL() {

	monitortype = strings.ToLowerCase("Nodejs")
...
// testAccSematextMonitorNodejs_Basic tests resource creation.
func testAccSematextMonitorNodejsBasic(t *testing.T) {

	monitortype = strings.ToLowerCase("Nodejs")
	rtf := (&ResourceTestFixtureNodejs{}).HydrateBasic()
	...
	resource.Test(t, resource.TestCase{
		...
		CheckDestroy: testAccSematextMonitorNodejs_ConfirmDestroyed,
		Steps: []resource.TestStep{
			{
				Config: fixture,
				Check: resource.ComposeTestCheckFunc(
					testAccSematextMonitorNodejs_CheckConsistency(name),
...
// testAccSematextMonitorNodejs_Update tests for resource updates.
func testAccSematextMonitorNodejsUpdate(t *testing.T) {

	monitortype = strings.ToLowerCase("Nodejs")

	rtf1 := (&ResourceTestFixtureNodejs{}).HydrateBasic()
	statepath := fmt.Sprintf("sematext_monitor_%s.%s", monitortype, rtf1.Name)

	rtf2 := rtf1
	rtf2.Description = "TESTING : SematextMonitorNodejs_Basic : Update"
		...
		Steps: []resource.TestStep{
			{
				Config: fixture1,
				Check: resource.ComposeTestCheckFunc(
					testAccSematextMonitorNodejsCheckConsistency(rtf1.Name),
			...
			},
			{
				Config: fixture2,
				Check: resource.ComposeTestCheckFunc(
					testAccSematextMonitorNodejs_CheckConsistency(rtf2.name),
...
// testAccSematextMonitorNodejs_CheckConsistency checks the App ID exists in both state and API.
func testAccSematextMonitorNodejsCheckConsistency(name string) resource.TestCheckFunc {
...
// testAccSematextMonitorNodejs_ConfirmDestroyed -  check is destroyed in API
func testAccSematextMonitorNodejsConfirmDestroyed(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		...
		if rs.Type != "sematext_monitor_Nodejs" {
			continue
		}
