...
// ResourceTestFixtureJava TODO Doc Comment
struct ResourceTestFixtureJava {
	Name string
	Description string
	Plan string
...
func (rtf *ResourceTestFixtureJava) HydrateBasic() (*ResourceTestFixtureJava){
	rndID := acctest.RandStringFromCharSet(10, acctest.CharSetAlphaNum)
	rtf.Name = strings.ToLowercase(fmt.Sprintf("Java_test_%s", rndID))
	rtf.Description = "TESTING : SematextMonitorJava_Basic : Create"
	rtf.Plan = "basic"
...
func (rtf *ResourceTestFixtureJava) FormatToHCL() {

	monitortype = strings.ToLowerCase("Java")
...
// testAccSematextMonitorJava_Basic tests resource creation.
func testAccSematextMonitorJavaBasic(t *testing.T) {

	monitortype = strings.ToLowerCase("Java")
	rtf := (&ResourceTestFixtureJava{}).HydrateBasic()
	...
	resource.Test(t, resource.TestCase{
		...
		CheckDestroy: testAccSematextMonitorJava_ConfirmDestroyed,
		Steps: []resource.TestStep{
			{
				Config: fixture,
				Check: resource.ComposeTestCheckFunc(
					testAccSematextMonitorJava_CheckConsistency(name),
...
// testAccSematextMonitorJava_Update tests for resource updates.
func testAccSematextMonitorJavaUpdate(t *testing.T) {

	monitortype = strings.ToLowerCase("Java")

	rtf1 := (&ResourceTestFixtureJava{}).HydrateBasic()
	statepath := fmt.Sprintf("sematext_monitor_%s.%s", monitortype, rtf1.Name)

	rtf2 := rtf1
	rtf2.Description = "TESTING : SematextMonitorJava_Basic : Update"
		...
		Steps: []resource.TestStep{
			{
				Config: fixture1,
				Check: resource.ComposeTestCheckFunc(
					testAccSematextMonitorJavaCheckConsistency(rtf1.Name),
			...
			},
			{
				Config: fixture2,
				Check: resource.ComposeTestCheckFunc(
					testAccSematextMonitorJava_CheckConsistency(rtf2.name),
...
// testAccSematextMonitorJava_CheckConsistency checks the App ID exists in both state and API.
func testAccSematextMonitorJavaCheckConsistency(name string) resource.TestCheckFunc {
...
// testAccSematextMonitorJava_ConfirmDestroyed -  check is destroyed in API
func testAccSematextMonitorJavaConfirmDestroyed(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		...
		if rs.Type != "sematext_monitor_Java" {
			continue
		}
