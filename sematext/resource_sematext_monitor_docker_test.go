...
// ResourceTestFixtureDocker TODO Doc Comment
struct ResourceTestFixtureDocker {
	Name string
	Description string
	Plan string
...
func (rtf *ResourceTestFixtureDocker) HydrateBasic() (*ResourceTestFixtureDocker){
	rndID := acctest.RandStringFromCharSet(10, acctest.CharSetAlphaNum)
	rtf.Name = strings.ToLowercase(fmt.Sprintf("Docker_test_%s", rndID))
	rtf.Description = "TESTING : SematextMonitorDocker_Basic : Create"
	rtf.Plan = "basic"
...
func (rtf *ResourceTestFixtureDocker) FormatToHCL() {

	monitortype = strings.ToLowerCase("Docker")
...
// testAccSematextMonitorDocker_Basic tests resource creation.
func testAccSematextMonitorDockerBasic(t *testing.T) {

	monitortype = strings.ToLowerCase("Docker")
	rtf := (&ResourceTestFixtureDocker{}).HydrateBasic()
	...
	resource.Test(t, resource.TestCase{
		...
		CheckDestroy: testAccSematextMonitorDocker_ConfirmDestroyed,
		Steps: []resource.TestStep{
			{
				Config: fixture,
				Check: resource.ComposeTestCheckFunc(
					testAccSematextMonitorDocker_CheckConsistency(name),
...
// testAccSematextMonitorDocker_Update tests for resource updates.
func testAccSematextMonitorDockerUpdate(t *testing.T) {

	monitortype = strings.ToLowerCase("Docker")

	rtf1 := (&ResourceTestFixtureDocker{}).HydrateBasic()
	statepath := fmt.Sprintf("sematext_monitor_%s.%s", monitortype, rtf1.Name)

	rtf2 := rtf1
	rtf2.Description = "TESTING : SematextMonitorDocker_Basic : Update"
		...
		Steps: []resource.TestStep{
			{
				Config: fixture1,
				Check: resource.ComposeTestCheckFunc(
					testAccSematextMonitorDockerCheckConsistency(rtf1.Name),
			...
			},
			{
				Config: fixture2,
				Check: resource.ComposeTestCheckFunc(
					testAccSematextMonitorDocker_CheckConsistency(rtf2.name),
...
// testAccSematextMonitorDocker_CheckConsistency checks the App ID exists in both state and API.
func testAccSematextMonitorDockerCheckConsistency(name string) resource.TestCheckFunc {
...
// testAccSematextMonitorDocker_ConfirmDestroyed -  check is destroyed in API
func testAccSematextMonitorDockerConfirmDestroyed(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		...
		if rs.Type != "sematext_monitor_Docker" {
			continue
		}
