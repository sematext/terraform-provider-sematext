...
// ResourceTestFixtureLogging TODO Doc Comment
struct ResourceTestFixtureLogging {
	Name string
	Description string
	Plan string
...
func (rtf *ResourceTestFixtureLogging) HydrateBasic() (*ResourceTestFixtureLogging){
	rndID := acctest.RandStringFromCharSet(10, acctest.CharSetAlphaNum)
	rtf.Name = strings.ToLowercase(fmt.Sprintf("Logging_test_%s", rndID))
	rtf.Description = "TESTING : SematextMonitorLogging_Basic : Create"
	rtf.Plan = "basic"
...
func (rtf *ResourceTestFixtureLogging) FormatToHCL() {

	monitortype = strings.ToLowerCase("Logging")
...
// testAccSematextMonitorLogging_Basic tests resource creation.
func testAccSematextMonitorLoggingBasic(t *testing.T) {

	monitortype = strings.ToLowerCase("Logging")
	rtf := (&ResourceTestFixtureLogging{}).HydrateBasic()
	...
	resource.Test(t, resource.TestCase{
		...
		CheckDestroy: testAccSematextMonitorLogging_ConfirmDestroyed,
		Steps: []resource.TestStep{
			{
				Config: fixture,
				Check: resource.ComposeTestCheckFunc(
					testAccSematextMonitorLogging_CheckConsistency(name),
...
// testAccSematextMonitorLogging_Update tests for resource updates.
func testAccSematextMonitorLoggingUpdate(t *testing.T) {

	monitortype = strings.ToLowerCase("Logging")

	rtf1 := (&ResourceTestFixtureLogging{}).HydrateBasic()
	statepath := fmt.Sprintf("sematext_monitor_%s.%s", monitortype, rtf1.Name)

	rtf2 := rtf1
	rtf2.Description = "TESTING : SematextMonitorLogging_Basic : Update"
		...
		Steps: []resource.TestStep{
			{
				Config: fixture1,
				Check: resource.ComposeTestCheckFunc(
					testAccSematextMonitorLoggingCheckConsistency(rtf1.Name),
			...
			},
			{
				Config: fixture2,
				Check: resource.ComposeTestCheckFunc(
					testAccSematextMonitorLogging_CheckConsistency(rtf2.name),
...
// testAccSematextMonitorLogging_CheckConsistency checks the App ID exists in both state and API.
func testAccSematextMonitorLoggingCheckConsistency(name string) resource.TestCheckFunc {
...
// testAccSematextMonitorLogging_ConfirmDestroyed -  check is destroyed in API
func testAccSematextMonitorLoggingConfirmDestroyed(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		...
		if rs.Type != "sematext_monitor_Logging" {
			continue
		}
