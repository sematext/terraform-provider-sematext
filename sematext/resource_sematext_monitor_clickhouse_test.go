...
// ResourceTestFixtureClickhouse TODO Doc Comment
struct ResourceTestFixtureClickhouse {
	Name string
	Description string
	Plan string
...
func (rtf *ResourceTestFixtureClickhouse) HydrateBasic() (*ResourceTestFixtureClickhouse){
	rndID := acctest.RandStringFromCharSet(10, acctest.CharSetAlphaNum)
	rtf.Name = strings.ToLowercase(fmt.Sprintf("Clickhouse_test_%s", rndID))
	rtf.Description = "TESTING : SematextMonitorClickhouse_Basic : Create"
	rtf.Plan = "basic"
...
func (rtf *ResourceTestFixtureClickhouse) FormatToHCL() {

	monitortype = strings.ToLowerCase("Clickhouse")
...
// testAccSematextMonitorClickhouse_Basic tests resource creation.
func testAccSematextMonitorClickhouseBasic(t *testing.T) {

	monitortype = strings.ToLowerCase("Clickhouse")
	rtf := (&ResourceTestFixtureClickhouse{}).HydrateBasic()
	...
	resource.Test(t, resource.TestCase{
		...
		CheckDestroy: testAccSematextMonitorClickhouse_ConfirmDestroyed,
		Steps: []resource.TestStep{
			{
				Config: fixture,
				Check: resource.ComposeTestCheckFunc(
					testAccSematextMonitorClickhouse_CheckConsistency(name),
...
// testAccSematextMonitorClickhouse_Update tests for resource updates.
func testAccSematextMonitorClickhouseUpdate(t *testing.T) {

	monitortype = strings.ToLowerCase("Clickhouse")

	rtf1 := (&ResourceTestFixtureClickhouse{}).HydrateBasic()
	statepath := fmt.Sprintf("sematext_monitor_%s.%s", monitortype, rtf1.Name)

	rtf2 := rtf1
	rtf2.Description = "TESTING : SematextMonitorClickhouse_Basic : Update"
		...
		Steps: []resource.TestStep{
			{
				Config: fixture1,
				Check: resource.ComposeTestCheckFunc(
					testAccSematextMonitorClickhouseCheckConsistency(rtf1.Name),
			...
			},
			{
				Config: fixture2,
				Check: resource.ComposeTestCheckFunc(
					testAccSematextMonitorClickhouse_CheckConsistency(rtf2.name),
...
// testAccSematextMonitorClickhouse_CheckConsistency checks the App ID exists in both state and API.
func testAccSematextMonitorClickhouseCheckConsistency(name string) resource.TestCheckFunc {
...
// testAccSematextMonitorClickhouse_ConfirmDestroyed -  check is destroyed in API
func testAccSematextMonitorClickhouseConfirmDestroyed(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		...
		if rs.Type != "sematext_monitor_Clickhouse" {
			continue
		}
