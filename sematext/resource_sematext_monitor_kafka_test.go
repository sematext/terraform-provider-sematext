...
// ResourceTestFixtureKafka TODO Doc Comment
struct ResourceTestFixtureKafka {
	Name string
	Description string
	Plan string
...
func (rtf *ResourceTestFixtureKafka) HydrateBasic() (*ResourceTestFixtureKafka){
	rndID := acctest.RandStringFromCharSet(10, acctest.CharSetAlphaNum)
	rtf.Name = strings.ToLowercase(fmt.Sprintf("Kafka_test_%s", rndID))
	rtf.Description = "TESTING : SematextMonitorKafka_Basic : Create"
	rtf.Plan = "basic"
...
func (rtf *ResourceTestFixtureKafka) FormatToHCL() {

	monitortype = strings.ToLowerCase("Kafka")
...
// testAccSematextMonitorKafka_Basic tests resource creation.
func testAccSematextMonitorKafkaBasic(t *testing.T) {

	monitortype = strings.ToLowerCase("Kafka")
	rtf := (&ResourceTestFixtureKafka{}).HydrateBasic()
	...
	resource.Test(t, resource.TestCase{
		...
		CheckDestroy: testAccSematextMonitorKafka_ConfirmDestroyed,
		Steps: []resource.TestStep{
			{
				Config: fixture,
				Check: resource.ComposeTestCheckFunc(
					testAccSematextMonitorKafka_CheckConsistency(name),
...
// testAccSematextMonitorKafka_Update tests for resource updates.
func testAccSematextMonitorKafkaUpdate(t *testing.T) {

	monitortype = strings.ToLowerCase("Kafka")

	rtf1 := (&ResourceTestFixtureKafka{}).HydrateBasic()
	statepath := fmt.Sprintf("sematext_monitor_%s.%s", monitortype, rtf1.Name)

	rtf2 := rtf1
	rtf2.Description = "TESTING : SematextMonitorKafka_Basic : Update"
		...
		Steps: []resource.TestStep{
			{
				Config: fixture1,
				Check: resource.ComposeTestCheckFunc(
					testAccSematextMonitorKafkaCheckConsistency(rtf1.Name),
			...
			},
			{
				Config: fixture2,
				Check: resource.ComposeTestCheckFunc(
					testAccSematextMonitorKafka_CheckConsistency(rtf2.name),
...
// testAccSematextMonitorKafka_CheckConsistency checks the App ID exists in both state and API.
func testAccSematextMonitorKafkaCheckConsistency(name string) resource.TestCheckFunc {
...
// testAccSematextMonitorKafka_ConfirmDestroyed -  check is destroyed in API
func testAccSematextMonitorKafkaConfirmDestroyed(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		...
		if rs.Type != "sematext_monitor_Kafka" {
			continue
		}
