...
// ResourceTestFixtureHadoop TODO Doc Comment
struct ResourceTestFixtureHadoop {
	Name string
	Description string
	Plan string
...
func (rtf *ResourceTestFixtureHadoop) HydrateBasic() (*ResourceTestFixtureHadoop){
	rndID := acctest.RandStringFromCharSet(10, acctest.CharSetAlphaNum)
	rtf.Name = strings.ToLowercase(fmt.Sprintf("Hadoop_test_%s", rndID))
	rtf.Description = "TESTING : SematextMonitorHadoop_Basic : Create"
	rtf.Plan = "basic"
...
func (rtf *ResourceTestFixtureHadoop) FormatToHCL() {

	monitortype = strings.ToLowerCase("Hadoop")
...
// testAccSematextMonitorHadoop_Basic tests resource creation.
func testAccSematextMonitorHadoopBasic(t *testing.T) {

	monitortype = strings.ToLowerCase("Hadoop")
	rtf := (&ResourceTestFixtureHadoop{}).HydrateBasic()
	...
	resource.Test(t, resource.TestCase{
		...
		CheckDestroy: testAccSematextMonitorHadoop_ConfirmDestroyed,
		Steps: []resource.TestStep{
			{
				Config: fixture,
				Check: resource.ComposeTestCheckFunc(
					testAccSematextMonitorHadoop_CheckConsistency(name),
...
// testAccSematextMonitorHadoop_Update tests for resource updates.
func testAccSematextMonitorHadoopUpdate(t *testing.T) {

	monitortype = strings.ToLowerCase("Hadoop")

	rtf1 := (&ResourceTestFixtureHadoop{}).HydrateBasic()
	statepath := fmt.Sprintf("sematext_monitor_%s.%s", monitortype, rtf1.Name)

	rtf2 := rtf1
	rtf2.Description = "TESTING : SematextMonitorHadoop_Basic : Update"
		...
		Steps: []resource.TestStep{
			{
				Config: fixture1,
				Check: resource.ComposeTestCheckFunc(
					testAccSematextMonitorHadoopCheckConsistency(rtf1.Name),
			...
			},
			{
				Config: fixture2,
				Check: resource.ComposeTestCheckFunc(
					testAccSematextMonitorHadoop_CheckConsistency(rtf2.name),
...
// testAccSematextMonitorHadoop_CheckConsistency checks the App ID exists in both state and API.
func testAccSematextMonitorHadoopCheckConsistency(name string) resource.TestCheckFunc {
...
// testAccSematextMonitorHadoop_ConfirmDestroyed -  check is destroyed in API
func testAccSematextMonitorHadoopConfirmDestroyed(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		...
		if rs.Type != "sematext_monitor_Hadoop" {
			continue
		}
