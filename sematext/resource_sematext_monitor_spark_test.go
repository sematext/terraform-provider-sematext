...
// ResourceTestFixtureSpark TODO Doc Comment
struct ResourceTestFixtureSpark {
	Name string
	Description string
	Plan string
...
func (rtf *ResourceTestFixtureSpark) HydrateBasic() (*ResourceTestFixtureSpark){
	rndID := acctest.RandStringFromCharSet(10, acctest.CharSetAlphaNum)
	rtf.Name = strings.ToLowercase(fmt.Sprintf("Spark_test_%s", rndID))
	rtf.Description = "TESTING : SematextMonitorSpark_Basic : Create"
	rtf.Plan = "basic"
...
func (rtf *ResourceTestFixtureSpark) FormatToHCL() {

	monitortype = strings.ToLowerCase("Spark")
...
// testAccSematextMonitorSpark_Basic tests resource creation.
func testAccSematextMonitorSparkBasic(t *testing.T) {

	monitortype = strings.ToLowerCase("Spark")
	rtf := (&ResourceTestFixtureSpark{}).HydrateBasic()
	...
	resource.Test(t, resource.TestCase{
		...
		CheckDestroy: testAccSematextMonitorSpark_ConfirmDestroyed,
		Steps: []resource.TestStep{
			{
				Config: fixture,
				Check: resource.ComposeTestCheckFunc(
					testAccSematextMonitorSpark_CheckConsistency(name),
...
// testAccSematextMonitorSpark_Update tests for resource updates.
func testAccSematextMonitorSparkUpdate(t *testing.T) {

	monitortype = strings.ToLowerCase("Spark")

	rtf1 := (&ResourceTestFixtureSpark{}).HydrateBasic()
	statepath := fmt.Sprintf("sematext_monitor_%s.%s", monitortype, rtf1.Name)

	rtf2 := rtf1
	rtf2.Description = "TESTING : SematextMonitorSpark_Basic : Update"
		...
		Steps: []resource.TestStep{
			{
				Config: fixture1,
				Check: resource.ComposeTestCheckFunc(
					testAccSematextMonitorSparkCheckConsistency(rtf1.Name),
			...
			},
			{
				Config: fixture2,
				Check: resource.ComposeTestCheckFunc(
					testAccSematextMonitorSpark_CheckConsistency(rtf2.name),
...
// testAccSematextMonitorSpark_CheckConsistency checks the App ID exists in both state and API.
func testAccSematextMonitorSparkCheckConsistency(name string) resource.TestCheckFunc {
...
// testAccSematextMonitorSpark_ConfirmDestroyed -  check is destroyed in API
func testAccSematextMonitorSparkConfirmDestroyed(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		...
		if rs.Type != "sematext_monitor_Spark" {
			continue
		}
