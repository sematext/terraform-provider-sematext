...
// ResourceTestFixtureAkka TODO Doc Comment
struct ResourceTestFixtureAkka {
	Name string
	Description string
	Plan string
...
func (rtf *ResourceTestFixtureAkka) HydrateBasic() (*ResourceTestFixtureAkka){
	rndID := acctest.RandStringFromCharSet(10, acctest.CharSetAlphaNum)
	rtf.Name = strings.ToLowercase(fmt.Sprintf("Akka_test_%s", rndID))
	rtf.Description = "TESTING : SematextMonitorAkka_Basic : Create"
	rtf.Plan = "basic"
...
func (rtf *ResourceTestFixtureAkka) FormatToHCL() {

	monitortype = strings.ToLowerCase("Akka")
...
// testAccSematextMonitorAkka_Basic tests resource creation.
func testAccSematextMonitorAkkaBasic(t *testing.T) {

	monitortype = strings.ToLowerCase("Akka")
	rtf := (&ResourceTestFixtureAkka{}).HydrateBasic()
	...
	resource.Test(t, resource.TestCase{
		...
		CheckDestroy: testAccSematextMonitorAkka_ConfirmDestroyed,
		Steps: []resource.TestStep{
			{
				Config: fixture,
				Check: resource.ComposeTestCheckFunc(
					testAccSematextMonitorAkka_CheckConsistency(name),
...
// testAccSematextMonitorAkka_Update tests for resource updates.
func testAccSematextMonitorAkkaUpdate(t *testing.T) {

	monitortype = strings.ToLowerCase("Akka")

	rtf1 := (&ResourceTestFixtureAkka{}).HydrateBasic()
	statepath := fmt.Sprintf("sematext_monitor_%s.%s", monitortype, rtf1.Name)

	rtf2 := rtf1
	rtf2.Description = "TESTING : SematextMonitorAkka_Basic : Update"
		...
		Steps: []resource.TestStep{
			{
				Config: fixture1,
				Check: resource.ComposeTestCheckFunc(
					testAccSematextMonitorAkkaCheckConsistency(rtf1.Name),
			...
			},
			{
				Config: fixture2,
				Check: resource.ComposeTestCheckFunc(
					testAccSematextMonitorAkka_CheckConsistency(rtf2.name),
...
// testAccSematextMonitorAkka_CheckConsistency checks the App ID exists in both state and API.
func testAccSematextMonitorAkkaCheckConsistency(name string) resource.TestCheckFunc {
...
// testAccSematextMonitorAkka_ConfirmDestroyed -  check is destroyed in API
func testAccSematextMonitorAkkaConfirmDestroyed(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		...
		if rs.Type != "sematext_monitor_Akka" {
			continue
		}
