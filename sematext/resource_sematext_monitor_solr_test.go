...
// ResourceTestFixtureSolr TODO Doc Comment
struct ResourceTestFixtureSolr {
	Name string
	Description string
	Plan string
...
func (rtf *ResourceTestFixtureSolr) HydrateBasic() (*ResourceTestFixtureSolr){
	rndID := acctest.RandStringFromCharSet(10, acctest.CharSetAlphaNum)
	rtf.Name = strings.ToLowercase(fmt.Sprintf("Solr_test_%s", rndID))
	rtf.Description = "TESTING : SematextMonitorSolr_Basic : Create"
	rtf.Plan = "basic"
...
func (rtf *ResourceTestFixtureSolr) FormatToHCL() {

	monitortype = strings.ToLowerCase("Solr")
...
// testAccSematextMonitorSolr_Basic tests resource creation.
func testAccSematextMonitorSolrBasic(t *testing.T) {

	monitortype = strings.ToLowerCase("Solr")
	rtf := (&ResourceTestFixtureSolr{}).HydrateBasic()
	...
	resource.Test(t, resource.TestCase{
		...
		CheckDestroy: testAccSematextMonitorSolr_ConfirmDestroyed,
		Steps: []resource.TestStep{
			{
				Config: fixture,
				Check: resource.ComposeTestCheckFunc(
					testAccSematextMonitorSolr_CheckConsistency(name),
...
// testAccSematextMonitorSolr_Update tests for resource updates.
func testAccSematextMonitorSolrUpdate(t *testing.T) {

	monitortype = strings.ToLowerCase("Solr")

	rtf1 := (&ResourceTestFixtureSolr{}).HydrateBasic()
	statepath := fmt.Sprintf("sematext_monitor_%s.%s", monitortype, rtf1.Name)

	rtf2 := rtf1
	rtf2.Description = "TESTING : SematextMonitorSolr_Basic : Update"
		...
		Steps: []resource.TestStep{
			{
				Config: fixture1,
				Check: resource.ComposeTestCheckFunc(
					testAccSematextMonitorSolrCheckConsistency(rtf1.Name),
			...
			},
			{
				Config: fixture2,
				Check: resource.ComposeTestCheckFunc(
					testAccSematextMonitorSolr_CheckConsistency(rtf2.name),
...
// testAccSematextMonitorSolr_CheckConsistency checks the App ID exists in both state and API.
func testAccSematextMonitorSolrCheckConsistency(name string) resource.TestCheckFunc {
...
// testAccSematextMonitorSolr_ConfirmDestroyed -  check is destroyed in API
func testAccSematextMonitorSolrConfirmDestroyed(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		...
		if rs.Type != "sematext_monitor_Solr" {
			continue
		}
